...
public class ListImageDownloader {
     ...
     */  
	public void imageDownload(String url,ImageView mImageView,String path,Activity mActivity,ListOnImageDownload download){
        ...
        String imageName = "";  
        if(url != null){  
            imageName = (Utility.getImageNameshequ(url));  //取名
        }  
       
        
        //check this 
        Bitmap bitmap = getBitmapFromFile(mActivity,imageName,path);  //从文件取图
        //先从软引用中拿数据  
        if(currBitmap != null && mImageView != null && softRefBitmap != null ){  
        	//mImageView.setImageBitmap(softRefBitmap);
       	//mImageView.setBackgroundDrawable(new BitmapDrawable(softRefBitmap));
        	download.onDownloadSucc(softRefBitmap,mImageView); 
       //     System.out.println("_______ mImageView.setImageBitmap(softRefBitmap); ");
        }  
        //软引用中没有，从文件中拿数据  
      ...
      // 	mImageView.setBackgroundDrawable(new BitmapDrawable(bitmap));
        ...
        //    System.out.println("_______ mImageView.setImageBitmap(bitmap);   ");
        }  
        else if(url != null && needCreateNewTask(mImageView)){  
            MyAsyncTask task = new MyAsyncTask(url, mImageView, path,mActivity,download);  
            if(mImageView != null){  
                Log.i(TAG, "执行MyAsyncTask --> " + Constants.DOWNLOAD_FLAG);  
                Constants.DOWNLOAD_FLAG ++;  
                task.execute();  
                //将对应的url对应的任务存起来  
                map.put(url, task);  
          //      System.out.println("_______download ");
            }  
	 ...
	 * @param download OnImageDownload回调接口，在onPostExecute()中被调用 
	 */  
	public void imageDownloadNoCache(String url,ImageView mImageView,String path,Activity mActivity,ListOnImageDownload download){  
        SoftReference<Bitmap> currBitmap = imageCaches.get(url);  //软引用缓存列表
        Bitmap softRefBitmap = null;  
        if(currBitmap != null){  
            softRefBitmap = currBitmap.get();//从软引用缓存列表取图  
        ...
        String imageName = "";  
        if(url != null){  
            imageName = Utility.getImageName(url);  //取名
        }  
        //check this 
        Bitmap bitmap = getBitmapFromFile(mActivity,imageName,path);  //从文件取图
        //先从软引用中拿数据  
...
//       // 	mImageView.setBackgroundDrawable(new BitmapDrawable(bitmap));
        ...
        if(imageName != null){  
            ...
            try {  
                if(Utility.hasSDCard()){  
                    real_path = Utility.getExtPath() + (path != null && path.startsWith("/") ? path : "/" + path);  
                }else{  
                    real_path = Utility.getPackagePath(mActivity) + (path != null && path.startsWith("/") ? path : "/" + path);  
                }  
                file = new File(real_path,imageName);  
           //     System.out.println(real_path+imageName);
                if(file.exists())  
    ...
    private boolean setBitmapToFile(String path,Activity mActivity,String imageName,Bitmap bitmap){  
        ...
        try {  
            ...
            file = new File(real_path, imageName);  
       //    System.out.println("--setBitmapToFile-----"+real_path);
            if(!file.exists()){  
                File file2 = new File(real_path + "/");  
                file2.mkdirs();  
            }  
            file.createNewFile();  //返回值为真假，如果已存在文件，返回假，如果路径不存在，抛出异常，问过文件z
    ...
    private class MyAsyncTask extends AsyncTask<String, Void, Bitmap>{  
        ...
        private String url;  
        private ListOnImageDownload download;  
        private String path;  
        private Activity mActivity;  
         
        public MyAsyncTask(String url,ImageView mImageView,String path,Activity mActivity,ListOnImageDownload download){  
        ...
        protected Bitmap doInBackground(String... params) {  
            ...
            if(url != null){  
                try {  
                    ...
                    String imageName = Utility.getImageName(url);  
        ...
        protected void onPostExecute(Bitmap result) {  
            //回调设置图片  
            if(download != null){  
            //    download.onDownloadSucc(result,url,mImageView,"/storage/emulated/0"+path+"/"+ Utility.getImageName(url));  
